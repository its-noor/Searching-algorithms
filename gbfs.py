...
def gbfs(source, dest, epolygons, tpolygons):
    queue = PriorityQueue()
    queue.push((source, [source]), heuristic(source, dest))
    visited = set()
    count = 0
    ...
    while not queue.isEmpty():
        ...
        if current == dest:
            print("GBFS found a path of length", len(path), "with", count, "nodes expanded")
            return path
        ...
        for nbr in get_neighbors(current, epolygons, tpolygons):
            if (nbr.x, nbr.y) not in visited:
                queue.push((nbr, path + [nbr]), heuristic(nbr, dest))

    print("GBFS did not find a path after", count, "nodes expanded")
    return None
